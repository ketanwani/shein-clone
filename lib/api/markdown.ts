...
export function buildMarkdown(baseUrl = DEFAULT_BASE_URL) {
  ...
  sections.push(
    [
      ...
      "",
      "**Two ways to name a shopper, and the token wins.** Send `X-Shopper-Email` with the shopper's address, or `Authorization: Bearer <token>` from the OTP flow. If both arrive the token is used. The bag, profile, wishlist and order history are all keyed by the resulting account, so every one of those calls — including the first add-to-bag — must name a shopper. There is no anonymous agent bag.",
      "",
      ...
      "- A **400** on a shopper-scoped call means no shopper was named — send `X-Shopper-Email` (or a bearer token) and retry. A **401** means the agent key is missing or wrong. Both are routine and recoverable; neither means the shopper needs a human.",
