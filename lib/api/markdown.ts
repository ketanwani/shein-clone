...
export function buildMarkdown(baseUrl = DEFAULT_BASE_URL) {
  ...
  sections.push(
    [
      ...
      "- **No refresh token is issued.** Store the expiry and re-run the OTP flow when it passes; a 401 `unauthorized` on a shopper-scoped call means the same thing. The expiry is published twice, as `data.expiresAt` (ISO 8601) and `data.expiresAtUnix` (seconds since the epoch, not milliseconds) — use whichever your platform can read.",
      "- **Send all three credentials on every call.** The token decides identity; `X-Customer-Ref` is reconciled against it, not weighed against it. The first call carrying both hands the ref's bag, addresses and history to the signed-in account, which is what lets you fill a bag before the shopper signs in and still check out with it. A ref that already belongs to a different account returns 409 `customer_ref_mismatch` — start a new conversation with a fresh ref rather than retrying.",
  ...
  sections.push(
    [
      ...
      "8. `POST /api/orders` with the test card `4242424242424242`, plus either `address_id` from step 6 or a full inline address. Send `Authorization: Bearer <data.token>` **and keep sending `X-Customer-Ref`** — that pairing is what hands the bag you built in step 4 over to the account. Converts the bag into an order and empties it. Add an `Idempotency-Key` header so a retry cannot buy twice.",
      ...
      "",
      "The bag in steps 4-6 is keyed by `X-Customer-Ref` and the order in step 8 by the signed-in account, so step 8 must carry **both** the ref and the token. The first call that does hands the ref's bag, saved addresses and profile over to the account permanently. Send only the token and step 8 finds an empty bag; send only the ref and it returns 401.",
    ].join("\n"),
  )
