...
export function buildMarkdown(baseUrl = DEFAULT_BASE_URL) {
  ...
  sections.push(
    [
      ...
      "| `X-Shopper-Email` | Which shopper is this for? | the agent, per shopper | every request |",
      "",
      "That is the whole credential set for an agent. There is no token, no sign-in step, nothing to store between calls and nothing to refresh.",
      "",
      "`X-Agent-Key` goes on every call. It proves the caller and nothing else.",
      "",
      "**Every shopper-scoped call takes `X-Shopper-Email`.** The bag, the profile and address book, the wishlist and the order history are all keyed by the account behind that address, so every one of those calls — including the first add-to-bag — must carry it. There is no anonymous agent bag.",
      "",
      "Addresses are trimmed and lowercased, so `Ada@Example.com` and `ada@example.com` are one shopper. An address seen for the first time provisions that shopper.",
      "",
      "> **⚠️ `X-Shopper-Email` is asserted by you and proves nothing.** Anyone holding `X-Agent-Key` can read or modify any shopper's data by naming their address — the same weakness `X-Customer-Ref` was removed for. The OTP bearer token is the correct mechanism and is still implemented; this exists only because the agent runtime cannot yet carry a token between calls. Off unless the server sets `ALLOW_SHOPPER_EMAIL_HEADER`, and acceptable only on a demo deployment with mock products and simulated payments.",
      "",
      ...
      "",
      "# The OTP flow still works and is unchanged, but nothing requires it.",
      "# Skip straight to step 2 unless you specifically want a session token.",
      ...
      "",
      "# 2. Everything shopper-scoped: agent key + the shopper's address.",
      ...
      "- A **400** on a shopper-scoped call means no shopper was named — send `X-Shopper-Email` and retry. A **401** means the agent key is missing or wrong. Both are routine and recoverable; neither means the shopper needs a human.",
  ...
  sections.push(
    [
      "A complete buy flow. Send `X-Agent-Key` on every call, and `X-Shopper-Email` from step 4 onwards. Steps 1-3 are public and need neither.",
      "",
      ...
      "4. `POST /api/cart/lines` with `{merchandiseId, quantity}` — fills the bag for the shopper named by `X-Shopper-Email`. Ask them for their email address before this point; it is the only thing you need from them.",
      "5. `GET /api/cart` — confirm lines and totals. Adjust with `PATCH /api/cart/lines` (quantity `0` removes a line).",
      "6. `GET /api/customer` — read `missing` to see which of `email`, `name` and `shipping_address` you still need to ask for, and `addresses` to offer a saved one.",
      "7. `POST /api/orders` with the test card `4242424242424242`, plus either `address_id` from step 7 or a full inline address. Converts the bag into an order and empties it. Add an `Idempotency-Key` header so a retry cannot buy twice.",
      "8. `GET /api/orders` — verify the order was recorded.",
      "",
      "On a return visit step 6 comes back with `missing: []`, so step 7 is an `address_id` and a card and nothing else. Nothing expires, so a returning shopper needs no extra step at all.",
    ].join("\n"),
  )
