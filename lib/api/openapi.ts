...
// Email first: this is the documented way, and generators surface the first entry.
const SHOPPER_REQUIREMENT = [BY_EMAIL, BY_TOKEN]
    ...
    components: {
      ...
      securitySchemes: {
        ...
        cartCookie: {
          ...
          name: "cartId",
          description:
            "Anonymous browser bag, set by the first POST /api/cart/lines and returned automatically by the browser. Adopted by the account on the first authenticated call. Not usable by an agent, which cannot carry a cookie between calls — an agent names the shopper with X-Shopper-Email instead.",
        },
      },
    },
  }
